...
def draw_tic_tac_toe(scr, items):  # функция крестика и нолика
    for i in range(3):
        for j in range(3):
            if items[i][j] == "0":
...
def get_win_check(fd, symbol):  # функция победы
    flag_win = False
    for line in fd:
        if line.count(symbol) == 3:
            flag_win = True
    for i in range(3):
        if fd[0][i] == fd[2][i] == symbol:
            flag_win = True
...
SCREEN_SIZE = (300, 300)  # размер окна

window = pygame.display.set_mode(SCREEN_SIZE)  # окно
...
while mainloop:  # глав цикл
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            mainloop = False

        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:  # нажатие на экран кнопкой мыши
            ...
            player_win = get_win_check(field, "x")  # надписи если я выиграл, проиграл, ничья
